...
class Omnifest:
    ...
    @classmethod
    def ensure(cls, deserialized_data: AnnotatedDict) -> None:
        ...
        targetless_keys = [
            f" * \"{key}\" in {deserialized_data[key].annotations.get('src')}"
            for key in deserialized_data.value
        ...
        if not target_available.value:
            raise NoTargetsError("input does not contain any targets")

    @property
    def tree(self) -> AnnotatedDict:
        return deepcopy(self._tree)

    @property
    def targets(self) -> AnnotatedDict:
    ...
    def as_target_string(self) -> str:
        ...
        target.ensure_valid(self._osbuild_ctx, self._tree)
        return target.as_string(self._osbuild_ctx, self._tree)


def _targets(tree: AnnotatedDict) -> AnnotatedDict:
    return AnnotatedDict({
        key.removeprefix(PREFIX_TARGET): val
        for key, val in tree.value.items() if key.startswith(PREFIX_TARGET)
    })
