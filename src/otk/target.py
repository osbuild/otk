...
class Target(ABC):
    @abstractmethod
    def ensure_valid(self, context: OSBuildContext, tree: Any) -> None: ...
...
class CommonTarget(Target):
    def ensure_valid(self, context: OSBuildContext, _tree: Any) -> None:
        pass
...
class OSBuildTarget(Target):
    def ensure_valid(self, context: OSBuildContext, tree: Any) -> None:
        if "version" in tree.value[PREFIX_TARGET + context.target_requested]:
            raise ParseError(
    ...
    def as_string(self, context: OSBuildContext, tree: Any, pretty: bool = True) -> str:
        osbuild_tree = tree[PREFIX_TARGET + context.target_requested].deep_dump()
