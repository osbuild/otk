...
from .annotation import AnnotatedNode, AnnotatedPath, AnnotatedList
from .error import CircularIncludeError


class State:

    def __init__(self, path: Optional[AnnotatedNode[pathlib.Path]] = None) -> None:
        if path is None:
            path = AnnotatedPath(pathlib.Path())

        if not isinstance(path, AnnotatedNode):
            # mainly for edge cases and tests
            path = AnnotatedPath(path)

        self.path = path
        self._define_subkeys: list[str] = []
        self._includes = AnnotatedList([])
        if path != pathlib.Path():
            self._includes.value.append(path)
    ...
    def copy(self, *, path: Optional[AnnotatedPath] = None, subkey_add: Optional[str] = None) -> "State":
        ...
        if path:
            ...
            if path in self._includes:
                circle = [p.fspath_with_include() for p in new_state._includes.value]
                circle.append(path.fspath_with_include())
                circle_str = ' ->\n'.join(circle)
                raise CircularIncludeError(f"circular include detected:\n{circle_str}")
            new_state.path = path
            new_state._includes.value.append(path)
        return new_state
