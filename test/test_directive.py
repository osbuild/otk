...
from otk.annotation import AnnotatedDict, AnnotatedInt
...
def test_op_map_join():
    ...
    d = AnnotatedDict({"values": [d1, d2]})

    assert op_join(ctx, state, d).deep_dump() == {"foo": "bar", "bar": "foo"}
