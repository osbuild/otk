...
from otk.annotation import AnnotatedNode, AnnotatedPath
...
def test_command_compile_on_base_examples(tmp_path, src_yaml, _mirror):
    ...
    expected = json.load(src_yaml.value.with_suffix(".json").open(encoding="utf8"))
...
def test_errors(src_yaml, _mirror, request):
    ...
    AnnotatedNode._basedirs = [request.config.invocation_dir, os.path.curdir]
