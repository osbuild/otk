...
def test_simple_sub_var():
    ...
    assert substitute_vars(context, state, AnnotatedStr("${my_var}")) == "foo"
...
def test_simple_sub_var_nested():
    ...
    assert substitute_vars(context, state, AnnotatedStr("${my_var}")) == [1, 2]
...
def test_simple_sub_var_nested_fail():
    ...
    test_list.annotations["src"] = "test.yml:1"
    context.define("my_var", test_list)

    data = AnnotatedStr("a${my_var}")
    data.annotations["src"] = "test.yml:2"

    expected_error = re.escape(
        f"test.yml:1 - expected int, float, or str. Can not use list to resolve string '{data}' (test.yml:2)"
    )
...
def test_sub_var_multiple_fail():
    ...
    test_str.annotations["src"] = "test.yml:1"

    context.define("a", test_str)

    test_list = AnnotatedList([1, 2])
    test_list.annotations["src"] = "test.yml:3"

    context.define("b", test_list)

    test_dict = AnnotatedDict({"one": 1})
    test_dict.annotations["src"] = "test.yml:4"
    context.define("c", test_dict)

    data = AnnotatedStr("${a}-${b}")
    data.annotations["src"] = "test.yml:5"

    # the a will be replaced but the b will cause an error
    expected_error = re.escape(
        r"test.yml:3 - expected int, float, or str. Can not use list to resolve string 'foo-${b}' "
    ...
    expected_error = re.escape(
        r"test.yml:4 - expected int, float, or str. Can not use dict to resolve string 'foo-${c}' "
        r"(variable from test.yml:1 applied to test.yml:5)"
    )
...
def test_substitute_vars_unhappy():
    ...
    assert ("foo.yaml: otk.define expects a <class 'otk.annotation.AnnotatedStr'>"
            " as its argument but received a `<class 'int'>") in str(
        exc.value)

    with pytest.raises(TransformDirectiveTypeError) as exc:
        var_template = AnnotatedStr("a${dict}b")
        var_template.annotations["src"] = "foo.yaml:1"
        substitute_vars(ctx, state, var_template)
    assert ("foo.yaml: expected int, float, or str. Can not use dict "
